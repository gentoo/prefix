...
if [ "$TEXINPUTS" ]; then
	if $(echo ${TEXINPUTS} | grep "${EPREFIX}"'/usr/share/texmf' &>/dev/null) ; then
		export TEXINPUTS=$(echo ${TEXINPUTS} | sed -e 's|'"${EPREFIX}"'/usr/share/texmf/*:\?||g')
	elif $(echo ${TEXINPUTS} | grep "${EPREFIX}"'/var/lib/texmf' &>/dev/null) ; then
		export TEXINPUTS=$(echo ${TEXINPUTS} | sed -e 's|'"${EPREFIX}"'/var/lib/texmf/*:\?||g')
	fi
fi
...
texconfig-sys font vardir "${EPREFIX}"/var/cache/fonts &>/dev/null
updmap-sys &>/dev/null
