...
python_version() {
	local tmpstr
	[[ -z ${PREFIX} ]] || python=${PREFIX}/usr/bin/python
...
#
python_makesym() {
	alternatives_auto_makesym "${PREFIX}/usr/bin/python" "python[0-9].[0-9]"
	alternatives_auto_makesym "${PREFIX}/usr/bin/python2" "python2.[0-9]"
}
...
python_mod_optimize() {
	local myroot
	# make sure ${PREFIX} isn't already appended and strip trailing /
	if [[ "${ROOT: -${#PREFIX}}" == "${PREFIX}" ]] ; then
		myroot="${ROOT%/}"
	else
		myroot="${ROOT%/}${PREFIX%/}"
	fi
	...
	python${PYVER} ${myroot}/usr/lib/python${PYVER}/compileall.py ${compileopts} ${PREFIX}$@
	python${PYVER} -O ${myroot}/usr/lib/python${PYVER}/compileall.py ${compileopts} ${PREFIX}$@
	eend $?
}
