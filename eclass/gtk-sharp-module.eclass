...
gtk-sharp-module_fix_files() {
	...
	local gapi_dir="${EROOT}/usr/share/gapi${GTK_SHARP_SLOT_DEC}"
...
gtk-sharp-module_src_install() {
	...
	then
		find "${ED}" -name '*.pc' -exec rm -rf '{}' '+' || die "la removal failed"
...
generate_pkgconfig() {
	...
	cat <<- EOF -> "${ED}/usr/$(get_libdir)/pkgconfig/${pkgconfig_filename}.pc"
		prefix=\${pcfiledir}/../..
		exec_prefix=\${prefix}
	...
	for dll in "${ED}"/usr/$(get_libdir)/mono/${pkgconfig_monodir}/*.dll
	do
		if ! [[ "${dll##*/}" == "policy."*".dll" ]]
		then
	...
	echo "${LSTRING}" >> "${ED}/usr/$(get_libdir)/pkgconfig/${pkgconfig_filename}.pc"

	if [[ "${pkgconfig_requires}" ]]
	then
		printf "Requires: ${pkgconfig_requires}" >> "${ED}/usr/$(get_libdir)/pkgconfig/${pkgconfig_filename}.pc"
	fi
	PKG_CONFIG_PATH="${ED}/usr/$(get_libdir)/pkgconfig/" pkg-config --silence-errors --libs ${pkgconfig_filename} &> /dev/null
	eend $?
}
