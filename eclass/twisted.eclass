...
twisted_src_test() {
	if [[ -n "${SUPPORT_PYTHON_ABIS}" ]]; then
		testing() {
			...
			local spath="${EPREFIX}$(python_get_sitedir)"
			mkdir -p "${T}/${spath}"
			...
			cd "${T}/${spath}" || die
			PATH="${T}${EPREFXI}/usr/bin:${PATH}" PYTHONPATH="${T}/${spath}" trial ${PN/-/.} || die "trial failed with Python ${PYTHON_ABI}"
			cd "${S}"
			rm -fr "${T}/${spath}"
		}
		...
		cd "${T}/${spath}" || die
		PATH="${T}${EPREFIX}/usr/bin:${PATH}" PYTHONPATH="${T}/${spath}" \
...
update_plugin_cache() {
	...
	rm "${EROOT}$(python_get_sitedir)/twisted/plugins/dropin.cache"
