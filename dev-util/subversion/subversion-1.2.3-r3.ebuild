...
src_compile() {
	local myconf
	myconf="--with-apr=${PREFIX}/usr --with-apr-util=${PREFIX}/usr"
...
#	use java && myconf="${myconf} $(use_with jikes)"
	...
	if use nowebdav; then
		...
		myconf="${myconf} --with-neon=${PREFIX}/usr"
	fi

	append-flags `${PREFIX}/usr/bin/apr-1-config --cppflags`
...
src_install () {
	...
	make DESTDIR=${DEST} install || die "Installation of subversion failed"
...
#	fi
	...
	dobin svn-config
	if use python; then
		make install-swig-py DESTDIR=${DEST} DISTUTIL_PARAM=--prefix=${DEST}  LD_LIBRARY_PATH="-L${D}/usr/$(get_libdir)" || die "Installation of subversion python bindings failed"
	...
	if use perl; then
		make DESTDIR=${DEST} install-swig-pl || die "Perl library building failed"
		fixlocalpod
	fi
	if use ruby; then
		make DESTDIR=${DEST} install-swig-rb || die "Installation of subversion ruby bindings failed"
	fi
	if use java; then
		make DESTDIR="${DEST}" install-javahl || die "installation failed"
	fi
...
pkg_config() {
	if [[ ! -x ${PREFIX}/usr/bin/svnadmin ]]; then
	...
	if [[ -e ${SVN_REPOS_LOC}/repos ]]; then
		...
		${PREFIX}/usr/bin/svnadmin create ${SVN_REPOS_LOC}/repos
