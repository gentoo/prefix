...
src_compile() {
	...
	econf ${myconf} \
		...
		$(use_with python) \
		$(use_enable nls) \
		--with-apr="${EPREFIX}/usr/bin/apr${apr_suffix}-config" \
		--with-apr-util="${EPREFIX}/usr/bin/apu${apr_suffix}-config" \
...
#	fi
	...
	dobin svn-config
	if use python; then
		make install-swig-py DESTDIR=${D} DISTUTIL_PARAM=--prefix=${ED} LD_LIBRARY_PATH="-L${ED}/usr/$(get_libdir)" || die "Installation of subversion python bindings failed"
