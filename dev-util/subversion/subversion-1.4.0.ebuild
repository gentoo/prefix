...
#	fi
	...
	dobin svn-config
	if use python; then
		make install-swig-py DESTDIR=${EDEST} DISTUTIL_PARAM=--prefix=${EDEST}  LD_LIBRARY_PATH="-L${D}/usr/$(get_libdir)" || die "Installation of subversion python bindings failed"
