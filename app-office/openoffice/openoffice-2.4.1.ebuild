...
# $Header: /var/www/viewcvs.gentoo.org/raw_cvs/gentoo-x86/app-office/openoffice/openoffice-2.4.1.ebuild,v 1.3 2008/06/12 00:24:21 ranger Exp $
...
src_install() {
	...
	use java && java-pkg_regjar "${ED}"/usr/$(get_libdir)/openoffice/program/classes/*.jar
...
pkg_postinst() {
	...
	use java && "${EPREFIX}"/usr/$(get_libdir)/openoffice/program/java-set-classpath $(java-config --classpath=jdbc-mysql 2>/dev/null) >/dev/null
