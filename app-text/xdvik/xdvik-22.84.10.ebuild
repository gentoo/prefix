...
src_install() {
	...
	cd ${S}/texk/xdvik
	einstall texmf="${ED}${TEXMF_PATH}" || die "install failed"

	mv "${ED}${TEXMF_PATH}/xdvi/XDvi" "${ED}etc/X11/app-defaults"
...
pkg_postinst() {
	if use emacs; then
		elog "Add"
		elog "	(add-to-list 'load-path (\"${EPREFIX}/usr/share/emacs/site-lisp/tex-utils\"))"
