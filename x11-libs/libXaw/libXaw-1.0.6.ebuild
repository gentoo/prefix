...
# $Header: /var/cvsroot/gentoo-x86/x11-libs/libXaw/libXaw-1.0.6.ebuild,v 1.2 2009/09/30 19:57:38 ssuominen Exp $
...
src_unpack() {
	...
	if [[ ${CHOST} == *-winnt* ]]; then
		PATCHES[${#PATCHES[*]}]="${FILESDIR}"/${PN}-1.0.5-winnt-cpp.patch
		PATCHES[${#PATCHES[*]}]="${FILESDIR}"/${PN}-1.0.5-winnt-no-libtool-hack.patch
		PATCHES[${#PATCHES[*]}]="${FILESDIR}"/${PN}-1.0.5-winnt-externalref.patch
	fi
