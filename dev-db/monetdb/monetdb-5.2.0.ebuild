...
# $Header: /var/cvsroot/gentoo-x86/dev-db/monetdb/monetdb-5.2.0.ebuild,v 1.2 2007/11/13 09:33:43 grobian Exp $
...
src_install() {
	...
	rm -f \
		"${ED}/usr/bin/Mapprove.py" \
		"${ED}/usr/bin/Mdiff" \
		"${ED}/usr/bin/Mfilter.py" \
		"${ED}/usr/bin/MkillUsers" \
		"${ED}/usr/bin/Mlog" \
		"${ED}/usr/bin/Mprofile.py" \
		"${ED}/usr/bin/Mtest.py" \
		"${ED}/usr/bin/Mtimeout" \
		"${ED}/usr/bin/prof.py" \
	...
	rm -Rf \
		...
		"${ED}/usr/lib/sql/Tests" \
		"${ED}/usr/share/MonetDB/Tests" \
		"${ED}/usr/share/sql/Tests" \
		|| die "removing tests"
	# remove windows cruft
	find "${ED}" -name "*.bat" | xargs rm -f || die "removing windows stuff"
