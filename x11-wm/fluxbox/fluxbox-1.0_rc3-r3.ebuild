...
# $Header: /var/cvsroot/gentoo-x86/x11-wm/fluxbox/fluxbox-1.0_rc3-r3.ebuild,v 1.2 2007/07/12 03:41:53 mr_bones_ Exp $
...
src_unpack() {
	...
	epatch "${FILESDIR}/${PV}/${P}-textproperties_segfault.patch" ||	die "Patch failed"

	# Some "bottom" windows, notably ROX Filer's panel, cause a flicker sometimes
	epatch "${FILESDIR}/${PV}/${P}-flicker.patch" ||	die "Patch failed"

	# Bug 176476 - Missing icons from fluxbox-generate_menu
	epatch "${FILESDIR}/${PV}/${P}-generate_menu_icon_fix.patch" ||	die "Patch failed"

	# Bug 181039 - Startup fails on first run
	epatch "${FILESDIR}/${PV}/startfluxbox_doesnt.patch" || die "Patch failed"
