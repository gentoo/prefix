...
src_compile() {
	...
	use aqua || myconf="${myconf} --disable-toolbox-glue" \
...
src_install() {
	...
	if use framework ; then
		...
		make DESTDIR="${DEST}" frameworkinstall || die "framework failed"
		if use aqua ; then
		...
		make DESTDIR="${DEST}" altinstall  || die "make altinstall failed"
	fi
	...
	dosed "s:/usr/lib/:${PREFIX}/usr/$(get_libdir)/:" /usr/bin/python-config
