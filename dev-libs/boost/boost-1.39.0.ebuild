...
KEYWORDS="~ppc-aix ~ia64-hpux ~amd64-linux ~x86-linux ~ppc-macos ~x64-macos ~x86-macos ~sparc-solaris ~sparc64-solaris ~x86-solaris ~x86-winnt"
...
_add_line() {
	if [ -z "$2" ] ; then
		...
		use debug && echo "${1}" >> "${ED}/usr/share/boost-eselect/profiles/${SLOT}/debug"
	else
