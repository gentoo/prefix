...
# $Header: /var/cvsroot/gentoo-x86/media-video/ffmpeg/Attic/ffmpeg-2.2.4.ebuild,v 1.2 2014/07/06 10:09:30 mgorny Exp $
...
src_prepare() {
	...
	# system headers on Darwin (stdio.h) define functions without
	# declaring the prototype first, so this will make the build fail
	if [[ ${CHOST} == *-darwin* ]] ; then
		sed -i -e 's/-Werror=missing-prototypes//' configure || die
	fi
}
