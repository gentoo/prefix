...
pkg_postinst() {
	...
	if [ ! -f "${PROOT}/var/lib/portage/world" ] &&
	   [ -f ${RPOOT}/var/cache/edb/world ] &&
	   [ ! -h ${PROOT}/var/cache/edb/world ]; then
		mv ${PROOT}/var/cache/edb/world ${PROOT}/var/lib/portage/world
		ln -s ../../lib/portage/world ${PROOT}/var/cache/edb/world
	fi

	for x in ${PROOT}/etc/._cfg????_make.globals; do
		# Overwrite the globals file automatically.
		[ -e "${x}" ] && mv -f "${x}" "${PROOT}/etc/make.globals"
	done
}
