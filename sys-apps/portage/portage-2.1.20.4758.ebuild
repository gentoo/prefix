...
pkg_postinst() {
	...
	# note: ROOT + EPREFIX = PROOT
	if [ ! -f "${ROOT}${EPREFIX}/var/lib/portage/world" ] &&
	   [ -f ${ROOT}${EPREFIX}/var/cache/edb/world ] &&
	   [ ! -h ${ROOT}${EPREFIX}/var/cache/edb/world ]; then
		mv ${ROOT}${EPREFIX}/var/cache/edb/world ${ROOT}${EPREFIX}/var/lib/portage/world
		ln -s ../../lib/portage/world ${ROOT}${EPREFIX}/var/cache/edb/world
	fi

	for x in ${ROOT}${EPREFIX}/etc/._cfg????_make.globals; do
		# Overwrite the globals file automatically.
		[ -e "${x}" ] && mv -f "${x}" "${ROOT}${EPREFIX}/etc/make.globals"
	done
}
